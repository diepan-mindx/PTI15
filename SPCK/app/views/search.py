...
class Search(QMainWindow):
    def __init__(self, root_ui_path, search_key):
        ...
        # bat su kien khi nhan vao item => thay doi du lieu o phan form
        self.ui.list_item.itemClicked.connect(self.on_item_clicked)

    def setCurrentItemDataForm(self, item_clicked):
    ...
    def create(self):
        ...
        # luu du lieu => edit
        if self.isEdit:
            self.note_controller.update_note(full_input_object)
        else:
            # luu duoi dang create
            self.note_controller.add_note(full_input_object)
    ...
    def delete(self):
        ...
        # lay full du lieu thong qua name
        full_current_object = self.note_controller.search_by_title(current_data)
        # thong bao hoi co xoa du lieu khong
        msg_box = QMessageBox()
        ...
        # neu khong dong y => k xoa => ket thuc ham
        if response == QMessageBox.StandardButton.No:
            return
        elif response == QMessageBox.StandardButton.Yes:
            # xoa du lieu trong data
            self.note_controller.delete_note(full_current_object)
            # load lai du lieu tren giao dien
            self.setListItem()
